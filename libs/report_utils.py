...
def format_report_header(writer: pd.ExcelWriter, df: pd.DataFrame, sheet_name: str = 'Sheet1') -> pd.ExcelWriter:
    """
    Function for formatting cells in a first row of XlsxWriter object (for all reports)
...
def colorize_companies_report(writer: pd.ExcelWriter, df: pd.DataFrame, sheet_name: str = 'Sheet1') -> pd.ExcelWriter:
    ...
    if df.shape[0] > 0:
        workbook = writer.book
        worksheet = writer.sheets[sheet_name]
        condition_format_1 = workbook.add_format({'bg_color': '#fa6464'})
        condition_format_2 = workbook.add_format({'bg_color': '#faed64'})
        condition_format_3 = workbook.add_format({'bg_color': '#64fa6e'})
        cells = f'D2:D{df.shape[0] + 1}'
        worksheet.conditional_format(cells, {
            'type': 'cell',
            'criteria': 'less than',
            'value': 0,
